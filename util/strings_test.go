...
func TestSplitStringAfterAny(t *testing.T) {
	cases := []struct {
		name  string
		s     string
		chars string
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			require.Equal(t, c.want, util.SplitStringAfterAny(c.s, c.chars))
		})
		t.Run(c.name+"_seq", func(t *testing.T) {
			require.Equal(t, c.want, slices.Collect(util.SplitStringAfterAnySeq(c.s, c.chars)))
		})
	}
}
